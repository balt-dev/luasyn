...
macro_rules! file {
    ($l: literal) => {[$l, include_str!($l)]};
}

static REAL_FILES: &[[&'static str; 2]] = &[
    file!("strontium.lua"),
    file!("omeganum.lua")
];
...
fn real_data() -> Result<(), Box<dyn Error>> {
    for [name, file] in REAL_FILES {
        let mut stream = match TokenStream::parse(*file) { Ok(v) => v,
        Err(e) => {
            let idx = e.index;
            ...
            panic!("tokenization error at {name}:{}:{}: {}\n{}\n{}^", lines + 1, idx - line_start, e.message, &file[line_start .. line_end], " ".repeat(idx - line_start))
        }};
